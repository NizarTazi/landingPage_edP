  ...
  (
    ...
    >
      ...
      <div className={clsx(className, 'wide-container')}>
        ...
        {description ? (
          <p className="mt-16 md:text-xl">{description}</p>
        ) : (
          descriptionComponent
        )}
        ...
        <LandingProductTour
          ref={ref}
          className={clsx('flex gap-16 w-full mt-12', className)}
          {...props}
        />
      </div>
    </section>
...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.Root
    ref={ref}
    className={cn('flex flex-col lg:flex-row gap-10 w-full', className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.Trigger
    ref={ref}
    className={cn(
      'rounded-md w-full lg:w-[420px] p-6 text-justify ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-neutral-500/10 data-[state=active]:text-foreground',
      className,
    )}
    {...props}
  />
